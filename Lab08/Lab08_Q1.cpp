...
private:
    ...
    TreeNode* deleteNode(TreeNode* node, int val) {
        if (val < node->data) {
            ...
            // 有兩個子樹的情況：找到右子樹的最小值來替代當前節點
            TreeNode* temp = findMin(node->right); // 找到右子樹的最小值
            node->data = temp->data; // 用最小值替代當前節點的資料
            node->right = deleteNode(node->right, temp->data); // 刪除最小值節點
        }
}
...
int main() {
    ...
    if (bst.search(deleteKey)) {
        cout << "值 " << deleteKey << " 已刪除。" << endl;
    }
    else {
        cout << "值 " << deleteKey << " 不存在於樹中，無法刪除。" << endl;
    }
