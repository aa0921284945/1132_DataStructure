...
int drawCard(Player* player) {/*傳入Player結構，用來判斷目前score*/
    int score = rand() % 10 + 1; /*發牌功能，呼叫一次發一張數值在1-10之間的牌*/
    if (score == 1) {/*如果抽到 1 點*/
        cout << "抽到 1 點，";
        if (player->score + 10 <= 21) {/*判斷會不會爆*/
            score += 10;/*如果不會爆，則選擇當作 11 點*/
...
void initializePlayer(Player* player, string name) {
    ...
    player->score = drawCard(player) + drawCard(player);/*呼叫抽卡函式兩次(兩張牌)，賦值給傳入的Player結構的score*/
}
// 顯示玩家分數
